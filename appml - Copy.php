...
if($action=="UPDATE"||$action=="DELETE"||$action=="ADD") {
    $logfile = fopen("logfile.txt", "a");
    fwrite($logfile, "Request Text : " . $txtRequest . "\n");
    fwrite($logfile, "User : " . $_SESSION["appmluser"] . "\n");
    fclose($logfile);
}
...
if ($cc != "OK") {
    $logfile = fopen("logfile.txt", "a");
    fwrite($logfile, "Security error : " . $security . "\n");
    fwrite($logfile, "CC : " . $cc . "\n");
    fclose($logfile);     
    app_error("APPML_ERR_NOT_AUTHORIZED");   
}
...
for ($j=0; $j<$fieldCounter; $j++) {
  if (strtolower($names[$j]) != strtolower(app_getProperty($updItem[$j], "item"))) {
      app_error("APPML_ERR_DATAMODEL");
    ...
    if (isset($modConfig->users)) {
      ...
      if (isset($modConfig->securitydb)) {
        ...
        if ($database != "") {
            ...
            try {
                if($debug) {
                    echo json_encode({"sql" : $sql, "username" : $params[1], "password" : $params[2]});
                    exit;
                }
                $dbRecordset=app_dbRead($dbConnection,$sql,$paramCount,$params);
            } catch {
            ...
            if ($totalCounter==0) {
                echo "Security SQL error : " . $sql . "\n";
                echo "Username : " . $userName . "\n";                
                echo "Password : " . $pwd . "\n";                
                exit;

                $logfile = fopen("logfile.txt", "a");
                fwrite($logfile, "Security SQL error : " . $sql . "\n");
                fwrite($logfile, "Username : " . $userName . "\n");                
                fwrite($logfile, "Password : " . $pwd . "\n");                
                fclose($logfile);                 
                app_error("APPML_ERR_NOT_AUTHORIZED");   
            }
            $row=$dbRecordset->fetch_array(MYSQLI_BOTH);
